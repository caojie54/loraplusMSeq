...
OUTPUT_BASE=${OUTPUT_BASE:-/mnt/dhwfile/raise/user/caojie/loraplusMSeq/outputs/medmcqa-full-outputonly}
PARTITION=${1:-raise}
EXPERIMENT=${2:-lora}
...
case "${EXPERIMENT}" in
  lora)
    script="exps/medmcqa/finetune_llama_lora_rank32_epoch2_medmcqa_full.sh"
    job_tag=lora
    epoch_tag=e2
    ;;
  lora_effective_update_pressure|effective_update_pressure)
    EXPERIMENT=lora_effective_update_pressure
    script="exps/medmcqa/finetune_llama_seq_alpha_effective_update_pressure_rank32_ratio0.01_candidate0.03_block50_uniform0.1_epoch1_medmcqa_full.sh"
    job_tag=eup
    epoch_tag=e1
    ;;
  dynamic_random)
    script="exps/medmcqa/finetune_llama_seq_dynamic_random_rank32_ratio0.01_block50_epoch1_medmcqa_full.sh"
    job_tag=dynamic
    epoch_tag=e1
    ;;
  *)
...
mkdir -p "${LOG_DIR}"
submission_log="${LOG_DIR}/submitted_medmcqa_${EXPERIMENT}_$(date +%Y%m%d_%H%M%S).tsv"
...
sbatch_args=(
  ...
  --quotatype=spot
  --requeue
  --job-name="mseq-medmcqa-${job_tag}-r32-${epoch_tag}-${PARTITION}"
  --ntasks-per-node=1
  --cpus-per-task="${CPUS_PER_TASK}"
  --gres=gpu:1
  --open-mode=append
  -o "${LOG_DIR}/medmcqa_${EXPERIMENT}_${PARTITION}_%j.log"
...
printf '%s\t%s\t%s\t%s\t%s\n' "${PARTITION}" medmcqa "${EXPERIMENT}" "${job_id}" "${output_root}" | tee -a "${submission_log}"
