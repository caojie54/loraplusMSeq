...
RUN_NAME=${RUN_NAME:-llama-3-1-8b-${METHOD}-qkvogateupdown-rank${RANK}-synthetic-text-to-sql-full-outputonly-epoch${NUM_TRAIN_EPOCHS}-loraopt${LORA_OPTIMIZER_DTYPE}-loralr${LORA_LR}}
...
TRAIN_SAMPLE_OFFSET=${TRAIN_SAMPLE_OFFSET:-0}
...
if [[ -f "${TRAIN_SUCCESS_MARKER}" ]]; then
  ...
  python train_medmcqa_and_synthetic_text_to_sql.py \
