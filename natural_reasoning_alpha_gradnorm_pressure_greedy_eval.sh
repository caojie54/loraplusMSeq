...
MODEL_DIR=${MODEL_DIR:-/mnt/dhwfile/raise/user/caojie/loraplusMSeq/outputs/natural-reasoning-20k/llama-3-1-8b-seq-alpha-gradnorm-pressure-qkvogateupdown-rank64-natural-reasoning-20k-epoch1-ratio0.02-block50-loraoptbf16-moduleoptfp32-resetoffload-loralr1e-4-modulelr1e-5}
EVAL_DATA_PATH=${EVAL_DATA_PATH:-/mnt/petrelfs/caojie1/projects/CoMoL/datasets/natural_reasoning_eval}
OUTPUT_DIR=${OUTPUT_DIR:-${MODEL_DIR}/eval_bs64_greedy_fixed_20260630}
...
echo "TEST_MAX_NEW_TOKENS=${TEST_MAX_NEW_TOKENS}"
echo "TEST_BATCH_SIZE=${TEST_BATCH_SIZE}"
echo "NATURAL_REASONING_BENCHMARKS=${NATURAL_REASONING_BENCHMARKS}"
...
if [[ "${REQUIRED_FREE_GB}" != "0" ]]; then
  ...
  IFS=, read -r gpu_id total_mem used_mem < <(
    nvidia-smi -i "${allocated_gpu}" --query-gpu=index,memory.total,memory.used --format=csv,noheader,nounits
  )
  gpu_id="${gpu_id//[[:space:]]/}"
...
# Greedy decoding is explicit here. test_natural_reasoning.py also passes
# use_model_defaults=False to generate(), so merged model generation_config cannot
# override this setting back to the LLM sampling defaults.
